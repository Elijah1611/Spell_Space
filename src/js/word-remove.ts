...
class WordRemove {
    ...
    public static async getWords() {
        try {
            const groupResult = await axios.get(`${process.env.HOST}/api/words/groups/${this.wordGroupId}`)
            ...
            docQuery('.remove-btn').addEventListener('click', async (e) => {
                ...
                if (markedForRemovalId){
                    ...
                    const test = await axios.delete(`${process.env.HOST}/api/words/${markedForRemovalId}`) 
                    console.log(test)
